...
#'
#' @import ggplot2 GenomicRanges IRanges
#' @importFrom scales comma
#' @importFrom stats approx
#' @export
plot_mht = function( grObj, wh ){

	# load recombination rate
#	dir <- system.file("extdata", package="qtlPlots")
    ...
    # gr_recomb = gr_recomb[seqnames(gr_recomb) %in% unique(seqnames(wh))] 
    gr_recomb = subset(gr_recomb, seqnames==unique(seqnames(wh)))

    # extact scores *within* the region wh
    gr_sub = grObj[grObj %within% wh]
    df_sub = with(gr_sub, data.frame(Position=start, score=score, inCandidateSet=inCandidateSet))
	...
	# if( (ord == 1) || ! showConditional ){
		fig = ggplot(df_sub, aes(Position, score, color=inCandidateSet)) + scale_color_manual(values = c("black", "red")) + theme_classic()
	# }else{		
	...
	fig = fig + geom_point(size=.4) + ylab(bquote(-log[10]~P)) + scale_x_continuous(expand=c(0,0), label=comma) + scale_y_continuous(expand=c(0,0), limits=c(0, ymax*1.05), sec.axis = sec_axis(~./(ymax.rate/100), name = "Recombination\nrate [cM/Mb]")) + geom_line(aes(y=rate*(ymax.rate/100)), color="dodgerblue", size=.5) + theme(legend.position="none")
